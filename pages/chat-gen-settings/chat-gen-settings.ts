...
export class ChatGenSettingsPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, private genProvider: GenericProvider, private toastCtrl: ToastController, private alertCtrl: AlertController, private viewCtrl:ViewController) {
  ...
  modify() {
    if( this.dataPost.topic && this.dataPost.description ){
      this.genProvider.postData(this.dataPost, "updateChat").then((result) => {
    ...
    let alert = this.alertCtrl.create({
      ...
      buttons: [
        ...
        },
        {
          text: 'Proceed',
          handler: () => {
            this.genProvider.postData(this.dataPost, "deleteMessages").then((result) => {
                this.responseData = result;
